...
Assume there is an element y of El(G) such that (y << (g1, H, G) and y << (g2, H, G)).
...
Let H be a subgroup of G.
Let g1, g2 be elements of El(G).
Inv(G)[g2]*^{G}  g1 << H Iff (g1, H, G) = (g2, H, G).

Axiom.
Let H be a subgroup of G.
Let g be an element of El(G).
Let h be an element of El(G).
Assume g is an element of (h, H, G).
Then (g, H, G) = (h, H, G).
...
Let x, y be elements of El(G) such that (x, P, G) = (y, P, G). 
Then (u *^{G} x, P, G) = (u *^{G} y, P, G).
Proof.
  Let us show that every element of (u *^{G} x, P, G) is an element of  (u *^{G} y, P, G).
    Let i be an element of (u *^{G} x, P, G).
    Take an element p of P such that i =  (u *^{G} x) *^{G} p.
    Then we have Inv(G)[u] *^{G} i =  x *^{G} p.
    Inv(G)[u] *^{G} i is an element of (x, P, G).
    Therefore Inv(G)[u] *^{G} i is an element of (y, P, G)
    and i is an element of (u *^{G} y, P, G).
  end.
qed.
...
such that f is from Prod(El(Gr(U, G)), s(P, G)) to s(P, G) and
...
Let P be a subgroup of G.
Let U be a subgroup of G.
Op(U, P, G) is a groupaction from Gr(U, G) on s(P, G).
Proof.
Take a function f such that f = Op(U, P, G).
Take a group H such that  H = Gr(U, G).
Take a set M such that M = s(P, G).
...
Let us show that for every element x of M for all elements a, b of El(H)
 f[(a *^{H} b, x)] =  f[(a,  f[(b, x)])].
  ...
  Take an element g of El(G) such that x = (g, P, G).

  We have f[(b, x)] = (b *^{G} g,P, G).

  f[(a, f[(b, x)])] = (a *^{G} (b *^{G} g),P, G).

  f[(a *^{H}  b, x)] = ((a *^{G} b)*^{G} g,P, G).
