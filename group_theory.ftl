...
Axiom Lagrange.
Let H be a subgroup of G.
Then |El(G)| = |H|*|LCosets(H, G)|.
###Proof (needs better def of numbers / cardinalities).
...
Axiom.
If p = a*b then a = 1 or a = p.

Axiom.
If p | a*b then p|a or p|b.

Axiom.
a | b iff there is a number k such that b = a*k.

Axiom.
p != 1.

Axiom.
If M is a subset of N and |M|=|N| then M = N.
...
Assume |El(G)| = p.
Let H be a subgroup of G.
Then H = {One(G)} or H = El(G).
Proof.
  (|H|) | p.
  Hence |H|=1 or |H|=p.
  If |H| = 1 then H = {One(G)}.
  If |H| = |El(G)| then H = El(G).
Qed.
...
Definition.
Let f be a grphom from G1 to G2.
Ker(f, G1, G2) = {j | j << El(G1) and f[j]=One(G2)}.
