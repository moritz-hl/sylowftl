...
Signature.
Let x be an element of El(G).
Pot(x, n, G) is an element of El(G).
...
Axiom.
Let x be an element of El(G).
Pot(x, Succ(n), G) = Mul(G)[(x, Pot(x, n, G))].
...
Definition.
Let x be an element of El(G).
Erz(x, G) = {y| (There is an number n1 such that y = Pot(x, n1, G)) or 
There is an number n2 such that y = Pot(Inv(G)[x], n2, G)}.
