...
Let G be a group.
Let U be a subgroup of G.
s(U, G) is disjunct collection.

Axiom Lagrange.
Let G be a group.
Let U be a subgroup of G.
card(Union(s(U, G))) = card(U)*card(s(U, G)).
...
If (g, Stab(x, f, G, M), G) = (h, Stab(x, f, G, M), G) then f[(g, x)] = f[(h, x)].
...
h is from s(Stab(x, f, G, M), G) to Orbit(x, f, G, M)
and (for all elements i of El(G) h[(i,Stab(x, f, G, M),G)] = f[(i, x)]).
...
Proof.
  Let us show that for every element y of Orbit(x, f, G, M) there is an element z of Dom(f) such that f[z] = y.
    ...
    Then we have h[(i,Stab(x, f, G, M),G)] = y.
  end.
end.

h is injective.
Proof.
  Let us show that for all elements h1, h2 of s(Stab(x, f, G, M), G) if h[h1] = h[h2] then h1 = h2.
    Let h1, h2 be elements of s(Stab(x, f, G, M), G) such that h[h1] =h[h2].
    
    Take elements g1, g2 of El(G) such that h1 = (g1,Stab(x, f, G, M),G) and h2 = (g2, Stab(x, f, G, M),G).
