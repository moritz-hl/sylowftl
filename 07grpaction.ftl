...
Proof.
  Let us show that every element of Coset(u *^{G} x, P, G) is an element of  Coset(u *^{G} y, P, G).
    Let i be an element of Coset(u *^{G} x, P, G).
    Take an element p of P such that i =  (u *^{G} x) *^{G} p.
    Then we have Inv(G)[u] *^{G} i =  x *^{G} p.
    Inv(G)[u] *^{G} i is an element of Coset(x, P, G).
    Therefore Inv(G)[u] *^{G} i is an element of Coset(y, P, G)
